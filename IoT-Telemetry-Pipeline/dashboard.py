...
<style>
    ...
    /* Status badge */
    .badge-ok     { color: #3fb950; font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; }
    .badge-warn   { color: #d29922; font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; }
    .badge-crit   { color: #f85149; font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; }
...
SUPABASE_BASE = "https://tolhsudmwhwpomuwqruv.supabase.co/rest/v1/device_telemetry"
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 🔍 Filter")
    # Will be populated after data fetch
...
# ── Device filter (sidebar, populated now) ────────────────────────────────────
...
if not critical_rows.empty:
    devices_in_crit = critical_rows["device_id"].unique()
    st.markdown(
        f'<div class="alert-critical">🚨 CRITICAL: High temperature detected on '
        f'{", ".join(devices_in_crit)} — check hardware immediately.</div>',
        unsafe_allow_html=True,
    )
elif not warn_rows.empty:
    devices_in_warn = warn_rows["device_id"].unique()
    st.markdown(
        f'<div class="alert-warn">⚠️ WARNING: Anomalous readings on '
        f'{", ".join(devices_in_warn)}.</div>',
        unsafe_allow_html=True,
    )
...
with m1:
    st.metric("🌡️ Latest Temperature", f"{latest['temperature']} °C", delta=f"{temp_delta:+.2f} °C")
with m2:
    st.metric("💧 Latest Humidity",    f"{latest['humidity']} %",    delta=f"{humid_delta:+.2f} %")
with m3:
    st.metric("📡 Active Devices",     len(all_devices))
with m4:
    st.metric("🔔 Alerts (shown)",     total_alerts,
              delta=None if total_alerts == 0 else f"{total_alerts} non-OK",
              delta_color="inverse")
...
chart_df["time_label"] = chart_df["created_at"].dt.strftime("%H:%M:%S")

tab_temp, tab_humid, tab_both = st.tabs(["🌡️ Temperature", "💧 Humidity", "📊 Combined"])

def make_line_chart(data, y_col, color, title, y_label):
    return (
        alt.Chart(data)
        ...
        .encode(
            ...
            color=alt.Color("device_id:N", title="Device",
...
with tab_temp:
    st.altair_chart(
        make_line_chart(chart_df, "temperature", "#f97583", "Temperature Over Time", "°C"),
        use_container_width=True,
    )

with tab_humid:
    st.altair_chart(
        make_line_chart(chart_df, "humidity", "#79c0ff", "Humidity Over Time", "%"),
        use_container_width=True,
    )
...
with tab_both:
    ...
    humid_layer = (
        alt.Chart(chart_df)
        .mark_line(strokeWidth=2, strokeDash=[4, 2])
        .encode(
            x="created_at:T",
    ...
    combined = (
        alt.layer(temp_layer, humid_layer)
...
display_df = display_df.rename(columns={
    "id": "ID", "created_at": "Timestamp", "device_id": "Device",
    "temperature": "Temp (°C)", "humidity": "Humidity (%)", "status_code": "Status",
})
